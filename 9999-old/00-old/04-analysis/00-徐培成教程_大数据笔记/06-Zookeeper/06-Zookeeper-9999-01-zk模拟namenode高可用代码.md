https://blog.csdn.net/zhongqi2513/article/details/92840447



* ⚠️：注意里面的子节点是临时节点，程序down机之后就会自动删除！！！
...
/**
 * #author      : ivanl001
 * #creator     : 2019-08-14 06:55
 * #description : 模拟HA的使用场景
 **/
 ...
 * 见代码注释
 */
public class A02_Namenode_HA_Simulation {

    // 连接信息
    private static final String CONNECT_STR = "centos01:2181,centos02:2181,centos03:2181";

    // 会话超时时长   会话建立成功最长的等待时间
    private static final int TIME_OUT = 50000;

    // 存储active namenode的父级znode节点
    private static final String ACTIVE_PARENT = "/ivanl001_namenode_active";
    // 存储standby namenode的父级znode节点
    private static final String STANBY_PARENT = "/ivanl001_namenode_standbys";
    // 锁节点
    private static final String LOCK_ZNODE = "/ivanl001_namenode_lock";

    // 当前上线的节点名称
    private static final String NAMENODE_HOST = "centos03";
                ...
                if (path.equals(ACTIVE_PARENT) && type == Event.EventType.NodeChildrenChanged) {
                    ...
                    try {
                        ...
                        if (onlyAtiveNM.size() == 0) {
                            ...
                            // 相当于已经有一个namenode把自己切换成为active了
                            // 所以不需要做什么操作
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
